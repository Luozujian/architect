...
#### 1.string类型底层数据结构有几种编码？(2_2024_12_14)
redis 2.+版本
...
#### 3. 为什么使用sds而不使用c自带的字符串？(2_2024_12_14)
1. o(1)时间获取字符串长度
2. 二进制安全
3. 杜绝缓冲区溢出
...
#### 4. hash底层编码包含哪些？(2_2024_12_14)
1. 压缩链表: 512个元素并且每个元素都小于64字节
2. hashtable: 不满足上述条件


#### 4. redis hash表的核心数据结构？(2_2024_12_14)
```c++
...
#### 5. redis字典结构包含哪些核心字段？(2_2024_12_14)
```c++
dictht ht[2];  两个hash表
int rehashidx;  rehash index，不进行rehash时为-1
```

#### 6. redis rehash可以缩容嘛？(2_2024_12_14)
可以，容量缩到第一个>= used的2^n次方

#### 7. redis扩缩容的条件？ (2_2024_12_14)
...
#### 8. redis也是渐进式hash嘛？ (2_2024_12_14)
是的,分散到每一次操作


#### 9. zset的底层编码？ (2_2024_12_14)
1. 压缩链表: 128个元素&每个元素小于64字节
2. 跳表: 不满足压缩链表条件



#### 9. 跳跃表接点包含哪些字段？ (2_2024_12_14)
```c++
level[].span 每层的跨度
level[].forward 前进指针
...
#### 10. 跳跃表的时间复杂度？(2_2024_12_14)
最好O(logn),最差o(n),平均也是o(logn)的时间复杂度

#### 11.set底层编码？(2_2024_12_14)
1. 整数集合: 数组并且元素个数小于512时整数集合，二次查找去重
2. hash表：不满足整数集合要求


#### 11. 什么时候会使用整数集合？(2_2024_12_14)
整数集合是集合的底层实现之一，当set元素不多，并且都是数字的时候，会使用整数集合

#### 12. 整数集合结构体包含哪些主要字段？(2_2024_12_14)
```c++
encoding 编码
length 长度
content[] 保存元素数组
...
#### 16. entryX数据节点包含哪些核心字段？ (2_2024_12_14)
```c++
