...
#### 2. 现代计算机如何弥补CPU与内存的速度差异呢？ (3_2023_10_31)
在CPU和内存之间增加多级缓存，L1,L2,L3。

#### 3. golang内存管理的page和操作系统的page有什么区别？ (3_2023_10_31)
page是由一组连续内存地址所组成的一个基本单位，在linux操作系统上它通常是4KB，
而在golang的内存管理中它通常是8Kb，两者并不一致，通常是倍数关系。

#### 4. 什么是span？ (3_2023_10_31)
...
mheap堆内存的抽象，当mcenter内存不够的时候，向mheap申请内存, mheap内存不够时，向os申请内存，向os申请内存是以OS的页为基本单位，需要做划分，放入到free二叉搜索树中去管理起来。
