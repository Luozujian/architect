...
#### 3. sync.cond 包含哪些方法以及用法？   (3_2023_10_18)
```c++
...
#### 4. sync.cond那些地方一定要加锁？  (3_2023_10_18)
1. wait方法一定要先加锁，因为wait方法里面会释放锁
2. 对条件的修改一定要加锁=>保证并发安全


#### 5. sync.cond 为什么不可复制?  (3_2023_10_18)
1. 因为sync.cond的结构体有指针，复制的时候发生浅拷贝，可能会发生无法预知的问题

#### 6. 为什么要用for 循环包裹wait()方法，if不行吗？  (3_2023_10_18)
如果使用if的话，当goroutine被唤醒的时候，其不一定满足条件，也会执行后续的代码

#### 7. atomic包提供了那些原子操作？ (3_2023_10_18)
1. ADD()
2. Load()
3. Store()
4. Swap()
