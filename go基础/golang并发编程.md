...
#### 4. sync.cond那些地方一定要加锁？  (2_2023_10_07)
1. wait方法一定要先加锁，因为wait方法里面会释放锁
2. 对条件的修改一定要加锁=>保证并发安全


#### 5. sync.cond 为什么不可复制?  (2_2023_10_07)
1. 因为sync.cond的结构体有指针，复制的时候发生浅拷贝，可能会发生无法预知的问题

#### 6. 为什么要用for 循环包裹wait()方法，if不行吗？  (2_2023_10_07)
如果使用if的话，当goroutine被唤醒的时候，其不一定满足条件，也会执行后续的代码

#### 7. atomic包提供了那些原子操作？  (2_2023_10_07)
1. ADD()
2. Load()
3. Store()
4. Swap()
...
#### 8. 什么是CAS操作？  (2_2023_10_07)
compare and swap 比较交换，其有3个值，当前内存值，旧值，新值，只有当内存值等于旧值的时候，才会把内存值修改成新值


#### 9. cas操作的优缺点？ (2_2023_10_07)
优点：
1. 乐观锁，避免加锁和解锁带来的开销
...
#### 10. 什么是sync.once?  (2_2023_10_07)
sync.once 使得函数无论被调用多次，但是只执行一次

#### 11. sync.once的结构体字段？ (2_2023_10_07)
```c++
type Once struct {
    done uint32
    m    Mutex
...
#### 12. Do()方法实现的大概逻辑？ (2_2023_10_07)
1. 判断done是不是等于0，如果等于0则表示未执行过，如果不等于0则表示已经执行过了
2. 没有执行过，加锁，然后执行函数，然后修改done的值为1

#### 13. 什么是waitGroup? (2_2023_10_07)
waitGroup是等待所有线程都执行完，再一起往下执行

#### 14. waitGroup有那些方法，如何使用？  (2_2023_10_07)
