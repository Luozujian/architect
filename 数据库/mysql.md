...
#### 1. InnoDb存储引擎有那几个隔离级别？ (4_2024_12_13)
1. 读未提交，存在胀读、不可重复读、幻读
2. 读已提交，不可重复读、幻读
3. 可重复读，可重复读 + 间隙锁 解决 大部分幻读问题，默认隔离级别
4. 串行化，无问题

#### 2. 什么是脏读、不可重复读、幻读？ (4_2024_12_13)
...
#### 7. mysql 是如何实现事物的隔离级别的？  (4_2024_12_13)
mysql 通过mvcc多版本控制来实现事物隔离级别，而MVCC多版本并发控制的具体实现依赖读视图和读视图的可见性算法



#### 8. 不同并发场景Mysql是如何解决的？  (4_2024_12_13)
数据库的并发场景有一下三种:
1. 读读，无并发问题
2. 读写，脏读，不可重复读和幻读问题
3. 写写，第一类丢失和第二类丢失问题
...
#### 9. Read View的可见性算法是怎么样的？  (4_2024_12_13)
...
#### 10. MVCC在隔离级别是可重复读时，是如何解决脏读和不可重复读的？  (4_2024_12_13)
在可重复读这个隔离级别下，根据Read View的可见性算法，其他还未提交的事务属于当前活跃事务ID列表中的事务，其锁修改的数据对当前事务不可见，由此解决了脏读问题

RR隔离级别下，Read View只在第一次快照读select生成读视图，第一次和第二次select之间，其他修改记录的事务只会属于当前活跃事务ID列表中的事务或者大于当前已分配最大事务ID，

根据可见性算法，对当前事务都是不可见的，由此解决了不可重复读问题。


#### 11. 读已提交这个隔离级别是如何实现的呢？  (4_2024_12_13)
每次快照读select都生成一份快照，属于第一个快照活跃事务ID列表中的事务Commit之后，就不在属于第二个快照活跃事务ID列表中的事务了，根据可见性算法，其所修改的数据对第二个快照是可见的，以此实现了RC隔离级别。

#### 12. 有几中锁模式？  (4_2024_12_13)
1. 行锁: 共享锁 (s锁)、排它锁(x锁)
2. 表锁: 意向锁共享锁 (IS)，意向排他锁(IX)，表级S锁，表级X锁
3. 间隙锁、临键锁(记录锁 + 间隙锁)，间隙锁与间隙锁不互斥，间隙锁不允许插入


#### 13. 意向锁解决了什么问题？  (4_2024_12_13)
